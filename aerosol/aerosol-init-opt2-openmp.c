...
#pragma omp parallel default(none) \
  shared(num, x, y, z, vx, vy, vz, gas, liquid, mass, gas_mass, liquid_mass, loss_rate) \
  shared(old_x, old_y, old_z) \
  shared(GRAVCONST, old_mass, k, T) \
  private(j, dx, dy, dz, d, F)
    {
#pragma omp for
...
#pragma omp for
    ...
    for(i=0; i<num; i++) {
	  ...
	  vz[i] += az;
	  */
	  factor = GRAVCONST * old_mass[j] / (d*d*d);
	  vx[i] = factor * dx + vx[i];
	  vy[i] = factor * dy + vy[i];
	  vz[i] = factor * dz + vz[i];
        }
      ...
      factor = sqrt(old_mass[i]*v_squared/mass[i])/sqrt(v_squared);
      */
      factor = sqrt(old_mass[i]/mass[i]);
      vx[i] *= factor;
      vy[i] *= factor;
      vz[i] *= factor;
    } // end of LOOP 2
