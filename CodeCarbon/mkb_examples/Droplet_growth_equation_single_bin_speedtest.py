...
# a mass of 120 g/mol and density 1400 km/m3. Store this information in an array
...
# Define the size of our particles [microns]
size_array = np.zeros((num_bins), dtype=float)
size_array[0] = 0.075 #microns
...
def dy_dt(array,t):
    ...
    # Initialise a size array. At each step we need to convert condensed
    # phase concentrations to a size for use in the transfer coefficient
    # and calculation of equilibrium pressure above the droplet
    size_array = np.zeros((num_bins), dtype=float)
...
def dy_dt_numba(array,t):
    ...
    # Now calculate a kelvin factor for every semi-volatile compound in this
    # size bin
    kelvin_factor=np.exp((4.0E0*mw_array*1.0e-3*sigma)/(R_gas*Temp_K*size*2.0e0*density))
    # Equilibrium pressure above droplets
    # Now calculate an equilibrium pressure [Pascals] for every compound in this
    # size bin
    Pressure_eq=kelvin_factor*mole_fractions*P_sat*101325.0e0
...
@track_emissions(project_name="dy_dt")
def time_func():
    odeint(dy_dt, array, t, rtol=1.0e-6, atol=1.0e-4, tcrit=None)

print("Python",timeit.Timer(time_func).timeit(number=100))

# Call the Numba function first to compile it
test=dy_dt_numba(array,0.0)
@track_emissions(project_name="dy_dt_numba")
def time_func():
    odeint(dy_dt_numba, array, t, rtol=1.0e-6, atol=1.0e-4, tcrit=None)

print("Numba",timeit.Timer(time_func).timeit(number=100))
